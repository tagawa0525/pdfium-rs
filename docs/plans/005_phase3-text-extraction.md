...
- `GraphicsState`: ctm (Matrix), text_matrix, text_pos (Point), text_line_pos, text_leading, text_rise, text_horz_scale（`Tz` 演算子のパーセント値を `/100` した小数で保持: 100% → 1.0）, text_state, font (Option<PdfFont>)
...
2. 変位 = `(width + char_space) * horz_scale` （スペース文字は `+ word_space`; `horz_scale` は1.0=等倍の小数）
3. ユーザー空間位置 = `CTM * text_matrix * (text_pos.x, text_pos.y + text_rise)`
4. `text_pos.x += 変位` で次の文字位置に進む
5. `TJ`の数値要素: `text_pos.x -= num / 1000.0 * font_size * horz_scale`（`horz_scale` は1.0=等倍）
